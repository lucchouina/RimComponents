...
{
    trxdbg(1,0,0,"Closing cli connection index %d fd %d\n", idx, cli[idx].fd);
    close(cli[idx].fd);
    cli[idx].fd=-1;
    /* free scope strdup()s */ 
    {
...
static void cmdList(int idx, int argc, char **argv)
{
    ...
    trxmgrClientWalkList(idx, listCb);
...
static void reportCb(int idx, int client, char *name)
{
    if(inScope(idx, client, name)){
        int ret=trxmgrClientAskReport(client, idx, cli[idx].tag, cli[idx].subCmd );
        cli[idx].waiting++;
...
char *fname=0;
    ...
    trxmgrClientWalkList(idx, reportCb);
}
...
{
    if(inScope(idx, client, name)){
        int newtag=trxmgrClientAskPush(client, idx);
...
{
    trxmgrClientWalkList(idx, incCb);
}

static void decCb(int idx, int client, char *name)
{
    if(inScope(idx, client, name)){
        int newtag=trxmgrClientAskPop(client, idx);
        cliPrt(idx, "%-10s : new tag is %d\n", name, newtag);
    }
}
...
{
    trxmgrClientWalkList(idx, decCb);
}
...
int val;
    ...
    if(inScope(idx, client, name)){
        val=trxmgrClientGetVar(client, cli[idx].curcmd);
        cliPrt(idx, "    %-10s : %d\n", name, val);
    }
}

static void getAppVar(int idx, char *varname, int cmd)
{
    cli[idx].curcmd=cmd;
    trxmgrClientWalkList(idx, getVarCb);
}
...
int val;
    ...
    if(inScope(idx, client, name)) {
        val=trxmgrClientSetVar(client, cli[idx].curcmd, cli[idx].curval);
        cliPrt(idx, "    %-10s : %s\n", name, val>=0?"ok":"Failed");
    }
}

static void setAppVar(int idx, char *varname, int cmd, int value)
{
    cli[idx].curval=value;
    cli[idx].curcmd=cmd;
    trxmgrClientWalkList(idx, setVarCb);
}
...
#define MAXARGS (sizeof(argv)/sizeof(argv[0]))
    ...
    if(!(cmd=getCmd(argv[0]))) {
        ...
        trxdbg(1,0,0,"Before execution of cmd '%s'\n", argv[0]);
        (*cmd->func)(idx, argc, argv);
        trxdbg(1,0,0,"After execution of cmd '%s'\n", argv[0]);
...
socklen_t len;
    ...
    if ((s=accept(cliFd, (struct sockaddr*)&addr, &len)) == -1)
        ...
        if(idx==MAXCLI) {
            ...
            trxdbg(1,0,0,"New Cli connection from '%s[%d]'\n", cli[idx].ipStr, idx);
            cli[idx].fd=s;
            cli[idx].ffd=-1;
            cli[idx].iac=0;
            ioctl(s, FIONBIO, &opt);
...
struct sockaddr_in addr;
    ...
    if((sock=socket(AF_INET, SOCK_STREAM, 0)) >= 0) {
        ...
        if(bind(sock, (struct sockaddr *) &addr, sizeof addr) >= 0) {
            if(listen(sock,64)) trxdbg(0,1,1, "CLI listen");
            cliFd=sock;
            for(i=0;i<MAXCLI;i++) cli[i].fd=-1;
            return;
        } else trxdbg(0,1,1, "CLI bind");
        close(sock);
    } else trxdbg(0,1,1, "CLI socket");
}
