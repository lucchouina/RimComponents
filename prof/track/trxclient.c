...
static int sendCmd2(int fd, uint32_t *seq, int cmd, int aux, int aux2)
{
int ret;

    sigprocmask(SIG_BLOCK, &sset, 0);
    if(sendCmdMore(fd, *seq, cmd, aux, aux2, 0, 0, 0))
        ret=recvAck(fd, seq);   
    else ret=-1;
...
static int sendCmd1(int fd, uint32_t *seq, int cmd, int aux)
{
    return sendCmd2(fd, seq, cmd, aux, 0);
}
...
int ret=-1;
    ...
    trxdbg(1,0,0,"[[ ClientSet ]]\n");
    switch(aux) {
        case CMD_POISON:     /* arg: bool on [0|1] */
            trxdbg(1,0,0,"Set POISON from %d to %d\n", poison, aux2);
            ret=poison;
            poison=aux2;
        break;
        case CMD_ENABLE:     /* arg: bool on [0|1] */
            trxdbg(1,0,0,"Set ENABLE from %d to %d\n", enable, aux2);
            ret=enable;
            enable=aux2;
        break;
        case CMD_VALIDATE:   /* arg: bool on [0|1] */
            trxdbg(1,0,0,"Set VALIDATE from %d to %d\n", validate, aux2);
            ret=validate;
            validate=aux2;
        break;
        case CMD_TRACK:      /* arg: bool on [0|1] */
            trxdbg(1,0,0,"Set TRACK from %d to %d\n", tracking, aux2);
            ret=tracking;
            tracking=aux2;
        break;
        case CMD_TAG:     /* arg : <tag value> */
            trxdbg(1,0,0,"Set TAG from %d to %d\n", alloctag, aux2);
            ret=alloctag;
            alloctag=aux2;
        break;
        default:
            // invalide command
            trxdbg(0,0,0,"Invalid set variable %d received\n", aux);
            return -1;
    }
    return ret;
}
...
static int clientGet(int aux)
{
    ...
    trxdbg(1,0,0,"[[ ClientGet ]]\n");
    switch(aux) {
        case CMD_POISON:
            trxdbg(1,0,0,"Get POISON %d\n", poison);
            return poison;
        break;
        case CMD_ENABLE:
            trxdbg(1,0,0,"Get ENABLE %d\n", enable);
            return enable;
        break;
        case CMD_VALIDATE:
            trxdbg(1,0,0,"Get VALIDATE %d\n", validate);
            return validate;
        break;
        case CMD_TRACK:
            trxdbg(1,0,0,"Get TRACK %d\n", tracking);
            return tracking;
        break;
        case CMD_TAG:
            trxdbg(1,0,0,"Get TAG %d\n", alloctag);
            return alloctag;
        break;
        default:
            // invalide command
            trxdbg(0,0,0,"Invalid get variable %d received\n", aux);
            return -1;
    }
}
...
{
    trxdbg(1,0,0,"ClientRcvCB : command %d received aux=%d more=%d\n", cmd->cmd, cmd->aux[0], more);
    switch(cmd->cmd) {
        ...
        default:
            // invalide command
            trxdbg(0,0,0,"Invalid command %d received\n", cmd);
            return -1;
        break;
    }
}
static void handleExchange()
{
    if(sock<0) return;   
    trxdbg(1,0,0,"handleExchange: waiting for messages.\n");
    while(rcvCmd(sock, clientRcvCB, 0)>=0 && needDoneTrigger){
        trxdbg(1,0,0,"handleExchange: got one more message needDoneTrigger[%d].\n",needDoneTrigger);
    }
}
...
struct sockaddr_un addr;
    ...
    if ((sock = socket(AF_UNIX, SOCK_STREAM, 0)) == -1) {
        trxdbg(0,1,0,"Client AF socket failure\n");
        return 0;
    }
    ...
    if (connect(sock, (struct sockaddr *)&addr, len) == -1) {
        close(sock);
        sock=-1;
        trxdbg(0,1,0,"Client AF connect failure\n");
        return 0;
    }
    needDoneTrigger=firstTime;
    trxdbg(1,0,0,"Sending REGISTER with pid %d\n", getpid());
    {
	int flags = fcntl(sock, F_GETFD);
	if (flags != -1)
	    fcntl(sock, F_SETFD, flags | FD_CLOEXEC);
    }    
    sendCmd1(sock, &seq, CMD_REGISTER, getpid());
    return 1;
}
...
restart:
    ...
    while(1) {
        ...
        sleep(1);
        trxdbg(1,0,0,"Trying to reconnect to server.\n");
...
{
    sendCmd2(sock, &seq, cmd, aux, aux2);
}
...
int clientInit()
{
    ...
    if(!connectToMgr(1)) {
        ...
        trxdbg(0,0,0,"Could not connect to trxmgr - Memory debug disabled.\n");
        trxdbg(0,0,0,"Please start trxmgr and restart this application.\n");
    }
    else {
        ...
        handleExchange();
        if(!enable) {
                trxdbg(1,0,0,"Not ENABLED - quitting!\n");
                close(sock);
                sock=-1;
                return 1;
        }
